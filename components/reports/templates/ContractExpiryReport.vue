...
<template>
  ...
  >
    ...
    <ReportSection title="Expiration Overview" boxed>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-4 rounded shadow-sm">
          ...
          <div class="text-2xl font-bold text-red-600">{{ expiringThisMonth }}</div>
        </div>
        <div class="bg-white p-4 rounded shadow-sm">
          <div class="text-sm text-gray-500">Expiring Next 90 Days</div>
          <div class="text-2xl font-bold text-amber-600">{{ expiringNext90Days }}</div>
        </div>
        <div class="bg-white p-4 rounded shadow-sm">
    ...
    <ReportSection title="Expiration Timeline" subtitle="Monthly contract expirations">
    ...
    <ReportSection title="Geographic Distribution" subtitle="Map of expiring contracts">
      <div v-if="loading" class="flex justify-center py-8">
        <UIcon name="i-heroicons-arrow-path" class="animate-spin h-8 w-8 text-gray-400" />
      </div>
      <div v-else-if="mapLocations.length === 0" class="text-center py-8 text-gray-500">
    ...
    <ReportSection title="Revenue Impact" subtitle="Monthly revenue at risk due to expirations">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <div class="bg-white p-4 rounded shadow-sm mb-4">
            ...
            <div class="text-2xl font-bold">{{ formatCurrency(totalRevenueAtRisk) }}</div>
            <div class="text-sm text-gray-500 mt-2">Over the next 12 months</div>
          </div>
          <div class="bg-white p-4 rounded shadow-sm">
    ...
    <ReportSection title="Expiring Contracts" subtitle="Details of contracts expiring in the selected period">
      <DataTable 
        :data="expiringContractsTable"
...
const mapLocations = computed(() => {
  ...
  return expiringContracts.value
    .filter(contract => contract.towers && 
...
const totalRevenueAtRisk = computed(() => {
  ...
  return expiringContracts.value.reduce((sum, contract) => sum + contract.monthly_rate, 0);
});
