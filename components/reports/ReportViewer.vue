...
<template>
  <div class="report-viewer">
    ...
    <div v-else-if="reportData">
      ...
      <div
        v-if="
          reportData.metadata && Object.keys(reportData.metadata).length > 0
        "
        class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6"
      >
        <div
          v-for="(value, key) in reportData.metadata"
          :key="key"
      ...
      <!-- Rendered sections -->
      <template
        v-else-if="reportData.sections && reportData.sections.length > 0"
      >
        <div
...
interface ReportAction {
  label: string;
  handler: () => void;
  color?: 'primary' | 'gray' | 'green' | 'yellow' | 'red' | 'blue'; // Use specific allowed colors
  icon?: string;
  disabled?: boolean;
  variant?: 'solid' | 'outline' | 'soft' | 'ghost' | 'link'; // Use specific variants
}
